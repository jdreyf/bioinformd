...
limma_contrasts_chunk <- function(grp.var, contr.v, path, limma.model=NULL, use_aw=TRUE, use_trend=FALSE,
  ...
  lc.r <- c(lc.r, paste0("write.csv(mtt.df, '", row.type, "_stats.csv')"))

  lc.txt <- c("## Test differential abundance",
              paste0("We test differential abundance of each row using limma [@limma]. The resulting CSV file is at ",
              rmd_links(filenames = paste0(row.type, '_stats.csv'), path = path), "."))
