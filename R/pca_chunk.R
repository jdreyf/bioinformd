...
pca_chunk <- function(grp.var, proj.nm, covars=NULL, elst=FALSE){
  if (elst){
    pca.r <- c(paste0("pc.coords <- ezpca(elst$E, pheno, color = '", grp.var, "', labels = TRUE, name = NA)"),
               paste0("ezpca(elst$E, pheno, color = '", grp.var, "', name = '", proj.nm, "_pca')"))
  } else {
    pca.r <- c(paste0("pc.coords <- ezpca(mtrx, pheno, color = '", grp.var, "', labels = TRUE, name = NA)"),
               paste0("ezpca(mtrx, pheno, color = '", grp.var, "', name = '", proj.nm, "_pca')"))
  }
  glue("write.csv(pca.coords$data, 'pca_coords.csv', na='')")
  ...
  pca.txt <- c(paste0("We visualize the clustering of samples in 2D using the first two principal components. ",
                     ...
                     "The PCA coordinates of the samples are at ", rmd_links('pca_coords.csv'), ".")
               )
