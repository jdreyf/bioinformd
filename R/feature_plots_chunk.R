...
feature_plots_chunk <- function(grp.var="grp", path, proj.nm, contr.v, use_annot=TRUE, row.type="gene", elst=FALSE){
  fp.r <- c(paste0("signif_hist(mtt, name='", proj.nm, "_signif_hist')"),
            paste0("multi_volcano(tab=mtt.df, name='", proj.nm,  "_volcanoes', ntop.sig = 3)"),
  ...
  fp.txt <- c(paste0("We plot the ", rows.type, " from the analysis. If there are multiple comparisons or associations, ",
            ...
            glue("The volcanoes are at ", rmd_links(filenames = paste0(proj.nm, '_volcanoes.pdf'), path = path),
                   ". Several ", row.type, "s with the lowest p-values or largest fold changes are annotated automatically. ",
                 "Unannotated points are plotted with some transparency. "),
