...
meanvar_trend_chunk <- function(voom.model, use_trend=FALSE, row.type="row"){
  ...
  mvt.txt <- c(paste0("We assess if there is a trend between a", row.type, "'s average abundance and its variance."))
  ...
  chunk <- c("# Assess mean-variance trend", "", "```{r mvt, include=TRUE, fig.height=4}", mvt.r, "```", "", mvt.txt)
  return(chunk)
}
